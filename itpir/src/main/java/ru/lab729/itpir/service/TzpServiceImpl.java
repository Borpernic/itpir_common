...
@Service
public class TzpServiceImpl implements TzpService {

    private final TzpRepository repository;

    @Autowired
    public TzpServiceImpl(TzpRepository repository) {
        this.repository = repository;
    }

    @Override
    public TzpEntity create(TzpEntity entity, int userId) {
        Assert.notNull(entity, "operator must not be null");
        return repository.save(entity, userId);
    }

    @Override
    public TzpEntity update(TzpEntity entity) throws NotFoundException {
    ...
    @Override
    public TzpEntity update(TzpEntity entity, int userId) {
        return checkNotFoundWithId(repository.save(entity, userId), entity.getId());
    }

    @Override
    public TzpEntity get(int id) throws NotFoundException {
